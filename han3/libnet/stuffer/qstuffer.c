/*
 *  $Id: qstuffer.c,v 1.3 2002/11/11 04:24:38 yudong Exp yudong $ 
 *
 *  Play 3-way HS first, then  
...
int main(int argc, char *argv[])
{
    ...
    char *srcfn = "src.txt";
    char *paybuf, *pp;
    int delay = 5; /* microsec between packets */
    int jd = 0;
    int jn = 0;
    int js = 0;
    int kd, kn;
...
for(i=0; i < js; i++) {  /* loop over all src IPs */ 
...
/* Building TCP (ACK, SYN) */
        ackn = libnet_get_prand(LIBNET_PRu32);   /* ACK number */ 
        t_op = libnet_build_tcp_options(
              "\x01\x01\x05\xac\x03\x02\x1b\x14\x00\x24\xd6\x2d\x01\x03\x03\x00\x01\x01\x04\x02\x02\x04\x05\xac\x00\x00\x00\x00", 
              24, 
              l,
              t_op); 
...
/* Building IP */
            ...
            ip = libnet_build_ipv4(
                LIBNET_IPV4_H + 24+ LIBNET_TCP_H, /* length */
                0x00,                                          /* TOS */
                0,                                            /* IP ID */
                0x4000,                                          /* IP Frag */
                64,                                         /* TTL */
