...
/** YDT inserted */
  FILE *fp;
  int len=40; /*max line lenth */
  int maxIPs=200; /*max number of lines */
  int ii = 0, i0;
  unsigned hashv, irand;
  static int jj=0;
  static char line[200][40];
  char tmp[40], mymsg[120];
  static time_t ct, lt=NULL;  /* current time and last file access time */
  char *cpt;
...
if (lt!=NULL && difftime(ct, lt) < 600.0) {   /* do nothing if within 10min */
} else {      /** read file and update status */

  jj = 0;
    if( (fp = fopen("yhip.txt", "r")) == NULL) {
      fprintf(stderr, "Error opening IP file yhip.txt\n");
      exit(1);
    } /* end if */
    while(fgets(tmp, len, fp) && jj < maxIPs) {
     strcpy(line[jj++], tmp);
    }
   lt=ct;
   fclose(fp);
    ...
    if( (fp = fopen("status.txt", "r")) == NULL) {
      fprintf(stderr, "Error opening IP file status.txt\n");
      exit(1);
    } /* end if */
    fgets(mymsg, len*3, fp);
   fclose(fp);

   /* change status message */
      yahoo_set_away (ylad->id,
                        99,
                        mymsg,
                        0);
       process_commands("MSG yudong1111 hello");  /* send a message to keep active */
}

/** set up random seed ***/
   cpt=who;
   for (hashv =0; *cpt !='\0'; cpt++)
     hashv = *cpt + 31 * hashv;

   srand(hashv);
/*** send out messages every 3lines per message */
   strcpy(mymsg, "MSG ");   /*empty it */
   strcat(mymsg, who); 
   strcat(mymsg, " Welcome! If no IP, try again"); 
       process_commands(mymsg);  /* send a one-liner */
   strcpy(mymsg, "MSG ");   /*empty it */
   strcat(mymsg, who); 
   strcat(mymsg, " "); 

   for (ii=0; ii< 2; ii++) { /* two segs of msgs. */
    for (i0=0; i0 < 3; i0++) { /* cat three lines */
     irand = (unsigned) (rand()*1.0/RAND_MAX*jj);
     strcat(mymsg, line[irand]);
    }

  /*  send_message(who, mymsg); */
   /* yahoo_send_im (ylad->id, NULL, who, mymsg, 0);  orig way of sending */
       process_commands(mymsg); 
   strcpy(mymsg, "MSG ");   /*empty it */
   strcat(mymsg, who); 
   strcat(mymsg, " "); 
   /* strcpy(mymsg, "");  empty it*/
   }  /* end of while */

   /* log the request */
/* oneway: 5/30/06 log to stdout
   if( (fp = fopen("yahoo.log", "a")) == NULL) {
      fprintf(stderr, "Error opening log file\n");
      exit(1);
   } */ 
    strncpy(tmp, asctime(localtime(&ct)), len);
    tmp[strlen(tmp)-1] = '\0'; /* remove \n */
    /* 
    fprintf(fp, "%s | REQ: %s\n", tmp,  who);
    fclose(fp);
    */
    fprintf(stdout, "%s | REQ: %s\n", tmp,  who);
...
/** YDT inserted */
  ...
  time_t ct;  /* current time */
   ...
   if( (fp = fopen("yahoo.log", "a")) == NULL) {
    ...
    strncpy(line, asctime(localtime(&ct)), len);
...
{
	int status = 0;   /*YDT available */
	int log_level = 1; /*YDT none */
	int lfd=0;
        char *uid; /*  = "dweb_ip";  */
        char *pwd; /*  = "psyah01p"; */
	...
	if (argc != 3) {
          fprintf(stderr, "Usage: yahoo uid passwd\n"); 
	  exit(1); 
	}
	uid = argv[1]; 
	pwd = argv[2]; 
...
/** inserted by YDT **/
        strcpy(ylad->yahoo_id, uid); 
        strcpy(ylad->password, pwd); 
/*** commented out by YDT ******************
	printf("Yahoo Id: "); 
