...
/** $Id: gethost-win.c,v 1.1 2004/08/08 18:03:44 yudong Exp yudong $***/
/***********************************************/
//#define WINDOWS
...
#define MAX_RETRIES 3   /* max retries of query */
...
int build_query(char *buf, char *domain, enum qtype, int qid); 
...
unsigned int getmx(char *domainp, char *dns_server_ip) {  
    ...
    len = build_query(qbuf, domain, MX, nid);  

    sockfd = socket(AF_INET, SOCK_DGRAM, 0); 
    /* multiple query out to increase reliability */
    for (kn=0; kn<MAX_RETRIES; kn++) 
      sendto(sockfd, qbuf, len, 0, (struct sockaddr *) &saddr, sizeof(saddr) ); 

    for(;;) {   /* keep getting replies until get the right one */
     ...
     if (alen > len ) { /*get some answer appended to query back */
       /* check if query id matches */ 
       memcpy(&aid, abuf, 2);  /* get query id in */
       if (aid != nid) continue ;  /* read new data */
       /* check hijack signatures */
       break; 
...
/*construct query data */ 
int build_query(char *qbuf, char *domain, enum qtype q, int nid) { 
