/*
 *  $Id: win-pa.c,v 1.1 2007/09/12 04:29:29 oneway Exp oneway $ 
 * Linux version
 *  Program to send data in TCP segs. Based on linux b-pack.c v1.2
 *  Usage:  
 *  win-pa -u -s src_IP -o src_port -d dst_IP -p dst_port -E dat_file -t ttl
...
int main(int argc, char *argv[])
{
    ...
    while ((c = getopt(argc, argv, "d:o:s:p:T:E:t:M:L:w:APRSFvuh")) != EOF)
    {
        switch (c)
        {
         case 'd':
...
usage(char *name)
{
    fprintf(stderr, "usage %s -u -s sIP -o sport -d dIP -p dport -t ttl -w win -T \"payload text\" -E data_file -M seq -L ack -A|P|R|S|F -v \n",
                name);
