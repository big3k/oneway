/*
 *  $Id: lpasyn.c,v 1.1 2003/04/29 02:03:56 yudong Exp yudong $
 *  Built upon lpa.c v 1.6, designed to syn a host 
 *  Program to send data in TCP segs. 
 *  Usage:  
 *  lpasyn -d dst_IP -p dst_port -t delay
...
usage(char *name)
{
    fprintf(stderr, "usage %s  -d dest_IP -p dport -t delay\n", name);
