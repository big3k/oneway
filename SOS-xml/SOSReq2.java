...
public class SOSReq2 {
 public static void main( String[] args ) {
   if (args.length != 2 ) {
     System.err.println("Usage: java SOSReq2 time outfile\r\nExample: java SOSReq2 2006-10-30T03:00:00Z /tmp/sm.dat\r\n"); 
     System.exit(-1); 
   } 
...
//      getData(args[0], args[1]);   // short-circuit for now YDT
 ...
 try {
   ...
   XMLStreamReader r=inputFactory.createXMLStreamReader(xml);

   while(r.hasNext()){

      int event = r.next();
            if (event==XMLStreamConstants.START_ELEMENT) {
                if(r.getLocalName() == ("values") ) { 
                  event = r.next(); 
                  rawdata = r.getText(); 
                  continue; 
                 }
