/*
 *  $Id: ipip_wrapper.c,v 1.1 2010/06/27 04:38:59 oneway Exp oneway $
 *  Based on "ipip_pptp.c, v 1.1" 
 *  Program to capture traffic specified by the filter and wrap it with ipip 
 *  Usage:  
 *  ./ipip_wrapper -i inf -d remote_ip -f "filter expression" 
 *  Example:
 *  ./ipip_wrapper -i eth0 -d 12.34.56.78 -f "dst port 1723"
 *
 *   gcc -o ipip_wrapper ipip_wrapper.c -lpcap 
 *
 */
...
int main(int argc, char *argv[])
{
    ...
    char filter[MAXTEXT];   /* filter, e.g.  "ip proto 47 or dst port 1723" */ 
    struct bpf_program fp;      /* hold compiled program     */
    bpf_u_int32 maskp;          /* subnet mask               */
    bpf_u_int32 netp;           /* ip                        */
    void usage(char *);

    while ((n = getopt(argc, argv, "i:d:f:")) != EOF)
    {
        switch (n)
        {
         case 'i':
         ...
         case 'd':
           ...
           break;
         case 'f':
           strncpy(filter, optarg, MAXTEXT);  /* filter */
           break;
         default:
                usage(argv[0]);
                exit(-1);
...
usage(char *name)
{
    printf("usage %s -i interface -d dest_ip -f \"filter expr\"\nExample:\n", name);
    printf("%s -i eth0 -d 12.34.56.78 -f \"src port 1234\"\n", name);
}
