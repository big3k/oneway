...
namespace CommonLib {
	...
	*/
	...
	void CMD5::Add(LPCVOID pData, DWORD nLength)
	{
		...
		if ( nLength >= partLen )
		{
			CopyMemory( &m_byBuffer[index], pInput, partLen );
		...
		CopyMemory( &m_byBuffer[index], &pInput[i], nLength - i );
	}
	...
	VOID CMD5::HashToString(char* szHash, const MD5* pHash, BOOL bURN)
	{
	//	char szHash[40] = { 0 };	
		sprintf(szHash, bURN
	...
	BOOL CMD5::HashFromString(LPCSTR pszHash, MD5* pMD5)
	{
	...
	BOOL CMD5::HashFromURN(LPCSTR pszHash, MD5* pMD5)
	{
	...
	void CMD5::CreateMD5Hash(char szHash[33], BYTE* pData, DWORD dwLen)
	{
		CMD5 md5;	
		MD5 md5Hash;
		md5.Add(pData, dwLen);
