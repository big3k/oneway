...
/* v1.0  Sun Oct 31 01:13:33 EDT 2010
   v1.1  5/10/2011: added "-r" option to support raw IP. 
   v1.2  5/16/2011: added "-u" option to support UDP. 
   v1.3  9/25/2011: added "-j" option to let user specify ip identification. 
...
int make_rawip(char *dbuf, 
              ...
              u_char ip_p,   u_short ip_id, u_char ttl,            // ip protocol & ttl
...
/* Make a UDP datagram, including IP and eithernet header. Take the parameters
   and return the length of the segment */
int make_udp(char *dbuf,
              u_char *smac,  u_char *dmac,
              u_long src_ip, u_long dst_ip,
              u_short src_prt, u_short dst_prt,
              u_short ip_id, u_char ttl,          
              char *payload, int paylen)
{
  ...
  ETHER_HDR *eth = (ETHER_HDR *) dbuf;
  struct ip *iph = (struct ip *) ( dbuf + sizeof(ETHER_HDR) );
  struct udphdr *udph = (struct udphdr *) ( dbuf + sizeof(ETHER_HDR) + sizeof(struct ip) );
  unsigned char seudo[MAXTEXT];
  UP_HDR upseudo_header;
 ...
 // Constructing ethernet header
 // Constructing IP and UDP headers
  iph->ip_hl = 5;
  iph->ip_v = 4;
  iph->ip_tos = 0;
  ip_len = sizeof (struct ip) + sizeof (struct udphdr);    /* no payload */
  iph->ip_id = htons(ip_id); 
  iph->ip_off = 0;
  iph->ip_ttl = ttl;
  iph->ip_p = 17;  // payload is UDP 
  iph->ip_sum = 0;
  iph->ip_src.s_addr = src_ip;
  iph->ip_dst.s_addr = dst_ip;
  udph->source_port = htons (src_prt);
  udph->dest_port = htons (dst_prt);
  udph->udp_length = htons( sizeof (struct udphdr ) + paylen ); 
  udph->udp_checksum = 0; 
  ...
  // udp checksum
 upseudo_header.source_address = src_ip;
 upseudo_header.dest_address = dst_ip;
 upseudo_header.placeholder = 0;
 upseudo_header.protocol = IPPROTO_UDP;
 upseudo_header.udp_length = htons( ip_len - sizeof(struct ip) );
 memcpy(&upseudo_header.udp, (u_char *)udph, sizeof(struct udphdr) );
 memcpy(seudo, &upseudo_header, sizeof(UP_HDR) );
 memcpy(seudo + sizeof(UP_HDR), (u_char *)udph+sizeof(struct udphdr),
      ip_len-sizeof(struct ip) - sizeof(struct udphdr) );

 udph->udp_checksum = in_checksum((unsigned short*)seudo,
       sizeof(UP_HDR) + ip_len - sizeof(struct ip) - sizeof(struct udphdr) );
...
int make_tcp(char *dbuf, 
              ...
              u_short src_prt, u_short dst_prt, 
              u_short win, u_short mss, u_short ip_id, u_char ttl, 
              u_short flags, char *payload, int paylen) 
{
 ...
 // Constructing IP and TCP headers
  ...
  if ( flags & 0x02  && mss ) { /* SYN is set and MSS > 0 */
    nmss = htons(mss); 
    tcph->th_ack = htonl(0); 
    memcpy(opt+2, &nmss, 2);  /* stuff mss in opt */
    memcpy(dbuf+sizeof(ETHER_HDR)+sizeof(struct ip)+sizeof(struct tcphdr), opt, 8);  /* Add option */ 
    ip_len += 8;   /* TCP option */ 
    tcph->th_off = 0x7; 
...
usage(char *name)
{
    fprintf(stderr, "\nUsage:\n win-pa2.exe -i dev_index -u -r ip_prot -j ip_id -s sIP -o sport -d dIP -p dport -t ttl -w win -T \"payload text\" -E data_file -M seq -L ack -A|P|R|S|F -O mss -v \n"); 
    ...
    fprintf(stderr, "-u          produce a UDP datagram instead of TCP segment \n"); 
    fprintf(stderr, "-O mss      specifies MSS in TCP option. If not given, no TCP option will be set in SYN\n");
    fprintf(stderr, "-j ip_id    specifies ip identification. If not given, take a random value\n");
...
int main(int argc, char *argv[])
{
   ...
   /* for ip headers */
    ...
    while ((c = getopt(argc, argv, "i:j:r:d:o:s:p:T:E:t:M:L:w:O:APRSFvuh")) != EOF)
    {
        switch (c)
        {
         case 'i':
...
/* Read the payload from file  */ 
   if (dmfn != NULL) { /* -- if -E option is given */
    ...
    for (i=0; i<pay_s; i++) printf("%02x ", (u_char)dms[i]);
    printf("\n\n");
